...
from libFSI import AdjointInterface as FSI # imports FSI python tools
...
def main():
    ...
    if myid == rootProcess:
        print('\n***************************** Initializing SU2 **************************************')
    try:
        FluidSolver = pysu2.CDiscAdjSinglezoneDriver(CFD_ConFile, 1, FSI_config['NDIM'], comm)
    ...
    if myid == rootProcess:
        print('\n***************************** Initializing pyBeam ************************************')
        try:
            SolidSolver = pyBeam_Interface.pyBeamADSolver(CSD_ConFile)
        except TypeError as exception:
