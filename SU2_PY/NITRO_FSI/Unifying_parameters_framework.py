	  ...
	  break
        ...
        if (not "=" in line) or (line[0] == '%'):  
         ...
         if this_param == "MACH_NUMBER":
                    ...
                    stringalt = 'UNST_TIMESTEP = '+ str(FSI_config['UNST_TIMESTEP']) + '   \n'
                    configfile2.write(stringalt)
         elif this_param == "UNST_TIME":
                    stringalt = 'UNST_TIME = '+ str(FSI_config['UNST_TIME']) + '   \n' 
                    configfile2.write(stringalt)
         ...
         #string values
         elif this_param == "UNSTEADY_SIMULATION":
                    if str(FSI_config['UNSTEADY_SIMULATION']) == 'YES':
                       stringalt = 'UNSTEADY_SIMULATION = '+ FSI_config['UNSTEADY_SCHEME'] + '   \n'    
                       configfile2.write(stringalt)  
                    else:   
                       stringalt = 'UNSTEADY_SIMULATION = '+ 'NO' + '   \n'                    
                       configfile2.write(stringalt)                     
         ...
         #For the NITRO_FRAMEWORK approach it is important to memorize the output file in precise locations                     
         elif this_param == "SURFACE_FLOW_FILENAME":
                    if FSI_config['CSD_SOLVER'] == 'NITRO_FRAMEWORK':
                       stringalt = 'SURFACE_FLOW_FILENAME = ' + FSI_config['OUTPUT_DIRECTORY'] + '/surface_flow'     + '   \n'
                       ...
                       configfile2.write(string)
         elif this_param == "VOLUME_FLOW_FILENAME":                    
                    stringalt = 'VOLUME_FLOW_FILENAME = ' + FSI_config['OUTPUT_DIRECTORY'] + '/flow'     + '   \n'                 
                    configfile2.write(stringalt)    
                    ...
                    configfile2.write(stringalt)        
         elif this_param == "SOLUTION_FLOW_FILENAME":                    
                    stringalt = 'SOLUTION_FLOW_FILENAME = ' + FSI_config['OUTPUT_DIRECTORY'] + '/restart_flow.dat'    + '   \n'                  
                    configfile2.write(stringalt)         
