    ...
    for (int i = 0; i < localNumberWetSurfaces; i++) {
      ...
      for (int iVertex = 0; iVertex < vertexSize[i]; iVertex++) {
        ...
        for (int iDim = 0; iDim < nDim; iDim++) {
          ...
          if (verbosityLevel_high) {
            cout << "Process #" << solverProcessIndex << "/" << solverProcessSize-1 << ": Initial coordinates of node (local index, global index, node color): (" << iVertex << ", " << iNode << ", " << geometry_container[ZONE_0][INST_0][MESH_0]->node[iNode]->GetColor() << "): " << coupleNodeCoord[iVertex][iDim] << endl; /*--- for debugging purposes ---*/
          }
        }
      }
...
void Precice::saveOldState( bool *StopCalc, double *dt ){
  ...
  for (int iPoint = 0; iPoint < nPoint; iPoint++) {
    for (int iVar = 0; iVar < nVar; iVar++) {
      ...
      solution_time_n_Saved[iPoint][iVar] = (solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->node[iPoint]->GetSolution_time_n())[iVar];
      solution_time_n1_Saved[iPoint][iVar] = (solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->node[iPoint]->GetSolution_time_n1())[iVar];
    }
    for (int iDim = 0; iDim < nDim; iDim++) {
      ...
      for (int jDim = 0; jDim < nDim; jDim++) {
        ...
        GridVel_Grad_Saved[iPoint][iDim][jDim] = (geometry_container[ZONE_0][INST_0][MESH_0]->node[iPoint]->GetGridVel_Grad())[iDim][jDim];
      }
    }
  }
...
void Precice::reloadOldState(bool *StopCalc, double *dt){
  for (int iPoint = 0; iPoint < nPoint; iPoint++) {
    ...
    for (int iDim = 0; iDim < nDim; iDim++) {
      for (int jDim = 0; jDim < nDim; jDim++) {
        geometry_container[ZONE_0][INST_0][MESH_0]->node[iPoint]->SetGridVel_Grad(iDim, jDim, GridVel_Grad_Saved[iPoint][iDim][jDim]);
      }
    }
  }
